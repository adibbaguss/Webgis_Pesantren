...
@elseif(Auth::User()->user_role == 'pelapor')
    <div class="scrollable-buttons  justify-content-md-center mb-2">
        <a href="{{ route('pelapor.madin.map_view') }}"
            class="btn btn-{{ request()->is('pelapor/madin/map_view*') ? 'success' : 'outline-success' }}">
            <i class="fas fa-map-marked-alt"></i> Umum
        </a>

        <a href="{{ route('pelapor.madin.map_facility') }}"
            class="btn btn-{{ request()->is('pelapor/madin/map_facility*') ? 'success' : 'outline-success' }}"
            disabled>
            <i class="fas fa-map-marked-alt"></i> Fasilitas
        </a>
    </div>
